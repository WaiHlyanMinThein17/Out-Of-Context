...
const sharedStyles = `
  ...
  @keyframes floatToSpace {
    ...
    60% {
      transform: translateY(-300px) translateX(100px) rotate(30deg);
      opacity: 0.6;
      filter: blur(2px);
    }
    100% {
  ...
  .results-card {
    animation: voteReveal 0.4s ease;
  }
  .imposter-badge {
...
function Voting({ gameId, myId, onGameEnd }) {
    ...
    try {
      ...
      if (data) {
        ...
        if (eliminated) {
          ...
          // Check if eliminated player is imposter
          if (eliminated.Imposter) {
            ...
            // Crewmate eliminated - Imposter wins!
            setTimeout(() => {
              setWinner('imposter');
              setShowEjection(false);
              setShowResults(true);
  ...
  useEffect(() => {
    if (timeLeft <= 0 && !showResults && !showEjection) {
      fetchResults();
      return;
    }
    const timer = setInterval(() => setTimeLeft(prev => prev - 1), 1000);
    return () => clearInterval(timer);
  }, [timeLeft, fetchResults, showResults, showEjection]);
  ...
  // Ejection Animation Screen - Floating into Space
  if (showEjection && ejectingPlayer) {
    return (
      <div style={{
        ...
        {/* Floating into Space Animation */}
        <div className="float-to-space" style={{
          position: 'absolute',
          bottom: '20%',
          left: '50%',
          ...
          <div style={{
            ...
            height: '200px',
            borderRadius: '50%',
            background: `radial-gradient(circle, ${ejectingPlayer.Imposter ? 'rgba(239,68,68,0.3)' : 'rgba(148,210,255,0.3)'}, transparent 70%)`,
            animation: 'pulse 1s ease-out',
          }} />
          
          <div style={{ marginBottom: '20px', position: 'relative' }}>
            <CrewmateIcon 
              color={getPlayerColor(ejectingPlayer.turn_order)} 
              size={120}
              isImposter={ejectingPlayer.Imposter}
            />
          </div>
          ...
          <div style={{
            ...
            fontWeight: '900',
            letterSpacing: '4px',
            color: ejectingPlayer.Imposter ? tok.red : tok.cyan,
            textShadow: `0 0 30px ${ejectingPlayer.Imposter ? 'rgba(239,68,68,0.8)' : 'rgba(148,210,255,0.8)'}`,
            marginBottom: '8px',
          }}>
        ...
        <div style={{
          ...
          right: '0',
          height: '100%',
          background: 'radial-gradient(ellipse at bottom, transparent 0%, rgba(0,0,0,0.5) 100%)',
          pointerEvents: 'none',
        }} />
      </div>
    );
  ...
  if (showResults) {
    const sortedResults = [...otherPlayers].sort((a, b) => 
      (voteTally[b.user_id] || 0) - (voteTally[a.user_id] || 0)
    );
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          background: `radial-gradient(circle, ${winner === 'crewmates' ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)'} 0%, transparent 70%)`,
          pointerEvents: 'none',
        }} />
        <div style={{
          position: 'absolute', bottom: '20%', right: '10%',
          width: '350px', height: '350px', borderRadius: '50%',
          background: `radial-gradient(circle, ${winner === 'crewmates' ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)'} 0%, transparent 70%)`,
          pointerEvents: 'none',
        }} />
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            ...
            <h1 style={{
              ...
              color: winner === 'crewmates' ? tok.green : tok.red,
              textShadow: `0 0 60px ${winner === 'crewmates' ? 'rgba(34,197,94,0.8)' : 'rgba(239,68,68,0.8)'}`,
            ...
            <div style={{
              width: '120px',
              height: '2px',
              background: `linear-gradient(90deg, transparent, ${winner === 'crewmates' ? tok.green : tok.red}, transparent)`,
              margin: '24px auto',
          ...
          {eliminatedPlayer && (
            <div style={{
              ...
              border: `1px solid ${eliminatedPlayer.Imposter ? tok.red : tok.borderBright}`,
              borderRadius: '12px',
              padding: '24px',
              marginBottom: '32px',
              textAlign: 'center',
              ...
              <div style={{ fontSize: '14px', color: tok.textDim, marginBottom: '12px' }}>
                THE CREW VOTED TO EJECT
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px' }}>
                <CrewmateIcon 
                  color={getPlayerColor(eliminatedPlayer.turn_order)} 
                  size={64}
                  isImposter={eliminatedPlayer.Imposter}
                />
                <div>
                  <div style={{ fontSize: '28px', fontWeight: '900', letterSpacing: '2px' }}>
                    PLAYER {eliminatedPlayer.turn_order + 1}
                  </div>
                  <div style={{ 
                    fontSize: '14px', 
                    color: eliminatedPlayer.Imposter ? tok.red : tok.green,
                    marginTop: '4px',
                  }}>
                    {eliminatedPlayer.Imposter ? '🔴 IMPOSTER' : '🟢 CREWMATE'}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div style={{ marginBottom: '32px', animation: 'fadeIn 0.5s ease 0.8s both' }}>
            <div style={{ fontSize: '12px', letterSpacing: '3px', color: tok.textDim, textAlign: 'center', marginBottom: '16px' }}>
              FINAL VOTE COUNT
            </div>
                ...
                return (
                  <div 
                    ...
                    style={{
                      ...
                      border: `1px solid ${isHighest ? (player.Imposter ? tok.red : tok.yellow) : tok.border}`,
                      borderRadius: '8px',
                      padding: '12px 20px',
                      animation: `fadeIn 0.3s ease ${1 + index * 0.1}s both`,
                    }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <CrewmateIcon color={getPlayerColor(player.turn_order)} size={32} />
                      <span style={{ fontSize: '16px', fontWeight: '700' }}>
                        PLAYER {player.turn_order + 1}
                      </span>
                      {player.Imposter && (
                        <span style={{ fontSize: '10px', color: tok.red, border: `1px solid ${tok.red}`, padding: '2px 8px', borderRadius: '4px' }}>
                          IMPOSTER
                        </span>
                      )}
                    </div>
                    <span style={{ fontSize: '24px', fontWeight: '900', color: isHighest ? (player.Imposter ? tok.red : tok.yellow) : tok.cyan }}>
                      {votes}
                    </span>
                  </div>
                );
          ...
          <div style={{ textAlign: 'center', animation: 'fadeIn 0.5s ease 1.2s both' }}>
            <button
              ...
              style={{
                ...
                textTransform: 'uppercase',
                cursor: 'pointer',
                background: winner === 'crewmates' ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)',
                color: winner === 'crewmates' ? tok.green : tok.red,
                border: `2px solid ${winner === 'crewmates' ? 'rgba(34,197,94,0.5)' : 'rgba(239,68,68,0.5)'}`,
                borderRadius: '8px',
                transition: 'all 0.2s ease',
              }}
              onMouseEnter={(e) => {
                e.target.style.background = winner === 'crewmates' ? 'rgba(34,197,94,0.35)' : 'rgba(239,68,68,0.35)';
                e.target.style.transform = 'scale(1.05)';
                e.target.style.boxShadow = `0 0 30px ${winner === 'crewmates' ? 'rgba(34,197,94,0.4)' : 'rgba(239,68,68,0.4)'}`;
              }}
              onMouseLeave={(e) => {
                e.target.style.background = winner === 'crewmates' ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)';
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '48px' }}>
          ...
          <h1 style={{
            ...
            VOTE
            <br />
            <span style={{ fontSize: '0.5em', letterSpacing: '8px', color: tok.red }}>
              THE IMPOSTER
            </span>
          </h1>
          <div style={{
        ...
        <div style={{
          ...
          margin: '0 auto 40px',
          background: 'rgba(6,15,31,0.8)',
          border: `1px solid ${timeLeft <= 10 ? 'rgba(239,68,68,0.4)' : tok.borderBright}`,
          borderRadius: '8px',
          padding: '20px',
          textAlign: 'center',
          animation: timeLeft <= 10 ? 'glowPulse 1s ease-in-out infinite' : 'none',
        }}>
          <div style={{ fontSize: '11px', letterSpacing: '3px', color: tok.textDim, marginBottom: '12px' }}>
            TIME REMAINING
          </div>
          <div style={{
            fontSize: '48px',
            ...
            color: timeLeft <= 10 ? tok.red : tok.cyan,
            textShadow: timeLeft <= 10 ? '0 0 30px rgba(239,68,68,0.6)' : '0 0 20px rgba(148,210,255,0.4)',
            animation: timeLeft <= 10 ? 'countdownUrgent 0.5s ease-in-out infinite' : 'none',
          }}>
        ...
        {loadingPlayers ? (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <div style={{
              ...
              borderTop: `2px solid ${tok.cyan}`,
              borderRadius: '50%',
              animation: 'spin 1s linear infinite',
              margin: '0 auto 16px',
            }} />
            <div style={{ fontSize: '11px', letterSpacing: '3px', color: tok.textDim }}>
              LOADING PLAYERS...
            </div>
          </div>
        ) : (
              ...
              {otherPlayers.map((player) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: isSelected ? 'rgba(34,197,94,0.05)' : 'rgba(6,15,31,0.6)',
                      border: `2px solid ${isSelected ? tok.green : tok.border}`,
                      borderRadius: '12px',
                      padding: '24px',
                      textAlign: 'center',
                  ...
                  >
                    ...
                    <div style={{ fontSize: '24px', fontWeight: '700', letterSpacing: '2px', marginBottom: '8px' }}>
                      PLAYER {player.turn_order + 1}
                    </div>
            ...
            <div style={{ textAlign: 'center' }}>
              <button
                ...
                style={{
                  ...
                  letterSpacing: '4px',
                  textTransform: 'uppercase',
                  cursor: (!selectedVote || hasVoted || isSubmitting) ? 'not-allowed' : 'pointer',
                  background: (!selectedVote || hasVoted || isSubmitting) 
                    ? 'rgba(148,210,255,0.1)' 
                    : 'rgba(34,197,94,0.2)',
                  color: (!selectedVote || hasVoted || isSubmitting) 
                    ? tok.textDim 
                    : tok.green,
                  border: `1px solid ${(!selectedVote || hasVoted || isSubmitting) 
                    ? tok.border 
                    : 'rgba(34,197,94,0.5)'}`,
                  borderRadius: '4px',
                  transition: 'all 0.2s ease',
